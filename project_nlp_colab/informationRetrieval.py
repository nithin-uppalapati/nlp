...
class InformationRetrieval():
	...
	def rank(self, queries):
		...
		# QE = np.zeros([len(t_collec),len(t_collec)])
		# for i in len(t_collec):
		# 	for j in len(t_collec):
		...
		for j in range(q_tot): #queries
			...
			# Add query expansion matrix here, to transform the query vector

			inter_vec = np.matmul(QE,q_vec)
			q_vec = inter_vec.copy()
...
'''
		...
		6. Check nDCG implementation
		7. Try query expansion:
